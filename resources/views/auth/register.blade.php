...
<head>
    <meta charset="UTF-8">
    <title>Register | Male Fashion</title>
            ...
            <div class="row justify-content-center">
                <div class="col-lg-6 col-md-8">
                    <div class="text-center mb-4">
                        ...
                        <div class="auth-title h2">Join Male Fashion</div>
                        <div class="text-muted mb-2">Create your account to start shopping</div>
                    </div>
        ...
        $(document).ready(function() {
            ...
            $('input[name="email"]').on('blur keyup', function() {
                ...
                if (email && email.indexOf('@') > -1) {
                    // Show checking state
                    emailField.addClass('checking-email').removeClass('error-input email-available');
                    emailField.siblings('.error').remove();
                    ...
                    emailCheckTimeout = setTimeout(function() {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.exists) {
                                    emailField.removeClass('checking-email email-available').addClass('error-input');
                                    emailField.siblings('.error').remove();
                                    emailField.after('<span class="error">This email is already registered. Please use a different email or <a href="{{ route("login") }}">sign in</a>.</span>');
                                } else {
                                    emailField.removeClass('checking-email error-input').addClass('email-available');
                                    emailField.siblings('.error').remove();
                                }
                            },
                            error: function() {
                                // If AJAX fails, let server-side validation handle it
                                emailField.removeClass('checking-email error-input email-available');
                                emailField.siblings('.error').remove();
                            }
                        });
                    }, 500); // Wait 500ms after user stops typing
                } else {
                    emailField.removeClass('error-input checking-email email-available');
                    emailField.siblings('.error').remove();
                }
            });
